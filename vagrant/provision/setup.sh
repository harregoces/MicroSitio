...
echo "Provisioning Micro Sitio virtual machine..."
...
curl -fsSL https://deb.nodesource.com/setup_5.x | bash > /dev/null 2>&1
...
cp /var/provision/config/nginx/micrositio_vhost /etc/nginx/sites-available/nginx_vhost
...
apt-fast install -y nodejs > /dev/null 2>&1
...
then
    ...
    echo "CREATE DATABASE micrositio" | mysql -uroot -p17959304

    echo "GRANT ALL ON micrositio.* TO 'unmentionable'@'localhost'" | mysql -uroot -p17959304
    echo "GRANT ALL ON micrositio.* TO 'unmentionable'@'%'" | mysql -uroot -p17959304
...
echo "Creating microsito tables"
php artisan migrate
